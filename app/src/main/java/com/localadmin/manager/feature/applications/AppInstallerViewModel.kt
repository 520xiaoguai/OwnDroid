...
class AppInstallerViewModel(
    ...
    fun initialize(intent: Intent) {
        ...
        uiState.update { it.copy(packages = it.packages + list.distinct()) }
        registerInstallerReceiver(application)
    }

    fun registerInstallerReceiver(context: Context) {
        ContextCompat.registerReceiver(
            context, broadcastReceiver, IntentFilter(ACTION), ContextCompat.RECEIVER_NOT_EXPORTED
        )
    }
    ...
    companion object {
        const val ACTION = "com.localadmin.manager.action.PACKAGE_INSTALLER_SESSION_STATUS_CHANGED"
    }
}
